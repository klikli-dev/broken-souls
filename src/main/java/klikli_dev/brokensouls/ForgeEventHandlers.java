...
import tao.brokensouls.data.LostChunkData;
import tao.brokensouls.data.LostSoulData;
import tao.brokensouls.varia.Tools;
                ...
                if (position.getY() >= miny && position.getY() <= maxy) {
                    ...
                    if (world.isAirBlock(new BlockPos(x, y, z))) {
                        ...
                        if (distance >= Config.MIN_SPAWN_DISTANCE) {
                            ...
                            Entity entity = EntityTools.createEntity(world, mob);
                            int cnt = world.getEntitiesWithinAABB(entity.getClass(), (new AxisAlignedBB(x, y, z, x + 1, y + 1, z + 1).expandXyz(8.0))).size();
